...
def generate_text(model, start_string, vocab, char2idx, idx2char):
    # Evaluation step (generating text using the learned model)

    # Number of characters to generate
    num_generate = 100

    splitted = prepare_text(start_string)
    cleaned = remove_unknowns(vocab, splitted)

    # Converting our start string to numbers (vectorizing)
    input_eval = [char2idx[s] for s in cleaned]
...
def main(training_from_scratch, filename, checkpoint_dir, checkpoint, epochs):
    ...
    text, char2idx, idx2char, vocab = preprocessing(text) # note that we are replacing the text here
    ...
    res = generate_text(model, start_string=u"Dafür dass dir wollen und können", vocab=vocab, char2idx=char2idx, idx2char=idx2char)
